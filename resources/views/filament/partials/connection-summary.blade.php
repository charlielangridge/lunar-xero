@php
    $authorized = filled($token?->access_token);
    $connected = $authorized && filled($tenant?->tenant_id);
@endphp
...
<div class="rounded-xl border border-gray-200 bg-white p-4 text-sm text-gray-700 shadow-sm">
    <div class="font-medium text-gray-950">
        {{ $connected ? 'Connected to Xero' : ($authorized ? 'Xero connected, select a tenant' : 'Not connected to Xero') }}
    </div>
